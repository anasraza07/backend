...
- schema = collection ke andr documents ka shape define krta hai yaani datatypes likhte hain ham like `name: String` phr saare documents ka yehi format hona chahiye jo collection ka schema defined hoga, schema eik blueprint hota hai ke ham apne collection mein is tarh ka data save krwayenge
- models = mongoose ki class hai jiski help se ham collection ke documents create, update, delete aur read krte hain yaani jb hamein collection ke liye documents create krna hai to sbse pehle to collection banega aur wo banta hai using model class kch is tarh  `const User = mongoose.model("User", userSchema);` 1st parameter ko collection ka naam denge jo capitalize ho aur singular hona chahiye phr mongodb isko small aur plural krke save krta hai automatically, aur 2nd parameter mein ham schema pass krte hain jo is collection ke saare documents mein follow hota hai, ab users naam ki eik collection bangai aur ye jo User ye model hai yaani eik tool jis se ab users collection ke documents banenge
- mongoose mein jo model class hai wo hamari collection ko represend krti hai aur us se banne waala obj hamare document ko represent krta hai
- insertOne = ab document banaane ke liye ham wohi model class yaani User use karenge jis se document banega like `const user1 = new User({name: "Anas"})`
- aur phr document ko db mein save krwane ke liye .save() method use hota hai jo asynchronous hai aur promise return krta hai so .then catch use krte hain for seeing results
- id ke sth hamein eik aur extra property milti hai mongoose mein `__V` ke naam se isko ham version smjh skte hain but abhi ignore kro
- insertMany = wese to zaada tar eik time mein eik hi insertion use hoti hai but multiple ke liye ham direct model ka use krte hain with method jese User.insertMany([{ name: "Tony" }, { name: "Peter" }]) aur phr .then lagaa ke resutl console log bhii kr skte hain
- operation buffering = ye mongoose ka feature hai jo ye use krta haii jiskii waja se ham db connect krne se pehle baaqi saare operations kr paate hain aur mongoose mongodb se connect hone ka wait nhi krta balke khud hi connect ke baad sb handle krta hai 
- model.find(), model.findOne(), model.findById() = ye teeno methods find ke liye, data get krne ke liye ye hote hain same usi tarh work krte hain jese in mongo shell aur ye methods promise return nhi krte balke query obj return krte hain lekin thenable hote hain yaani inke aagay bhi .then lagta hai lag skta hai
- findById method isliye hai bcuz id ko wese bhi bht zaada use kia jaata hai databases mein for query
- update in mongoose = yahan model.updateOne() aur model.updateMany() methods use hote hain jis mein pehle filter kro kisi condition ki base pr aur next object mein update kro with key-value pair jo update krni hai so return mein metadata obj milta hai wohi jo shell mein aata hai modified counts wagera
- update krne ke baad bhi agr return value mein data chahiye to ham findAndUpdate() aur findByIdAndUpdate() methods ka use krte hain jo by default original data return krta hai `new` ki value false hoti hai so modified data ke liye ham 2nd parameter ke baad eik aur obj add krte hain is tarh {new: true} jis se phr hamein modified data milta hai
- delete = is ke liye bhi 2 methods hain model.deleteOne() aur model.deleteMany() same syntax same tarh work krte hain lekin return mein metadata dete hain so again, return mein deleted data access krne ke liye 2 methods aur hain wohi update waale `findOneAndDelete()` aur `findByIdAndDelete()` iska bhi same syntax aur wese hi work krte hain
- schema validations = yaani rules jo ham set krte hain schema banaate waqt jese sql mein constraints hote hain same wese hi rules ham schema mein bhi set krte hain aur datatype khud bhi constraint hi hai bcuz String set krna ka mtlb hai ke ab string mein hi value store honi chahiye db mein
- schema banaane ka eik tareeqa to bilkul simple hai just datatypes define krdo but agr bht saare constraints hon to eik eik data key ka obj banta hai aur us mein schema types aati hain jese type, required, etc. is tarh ham bht saare rules eik hi key pr set kr skte hain
- required: true = `not null` constraint in sql (same chz hain dono)
- datatype defined krne ka mtlb ye nhi hota ke String mein hi value aaye balke String mein hi value store ho jese agr kisi key ko number type dia hai aur mongodb number mein convert nhi kr paaraha to cast error dega 
- cast error means parse error, db aapki value ko number mein convert nhi kr paa raha but number in string ho to koi error nhi aata
- schemaType options to bht saare hain but zaada use hone waale kch ye hain jese: default, required, min, max, minLength, maxLength, enum, immutable, string ke liye lowercase, uppercase, trim etc
- datatype mein array of string define krne ke liye ham kch yu likhte hain: `[String]`